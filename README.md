...
当前代码已经形成 **V0.2 BYOK 本地工程链路**：当前页经 V0.1 capture bridge 进入 SwiftUI 后，用户可以保存单个 OpenAI-compatible 配置，使用 Keychain 中的 API Key 对当前正文执行总结或翻译，并看到 streaming、停止、完成、不完整和失败状态。任务 D 已统一中文错误恢复动作，并增加运行时 redaction 与独立 secret hygiene 门禁。这个结论只表示 V0.2 工程验收完成，不表示完整 P0、发布准备或产品发布已经完成；设置页连接测试、SQLite、历史、导出、Q&A、多 Provider、真实 Provider 自动测试、签名、公证和发布包仍未实现。
...
- V0.1 自动化垂直链路已建立：语言中立 JSON Schema、共同 fixtures、WXT MV3 扩展、SwiftUI APP、Native Host、Unix socket 与结构化错误均可构建和测试；Chrome、Brave 150 的真实触发、离线错误、在线接收与 20 次 Release p95 已通过，Edge 仍待收尾。
...
下一步使用 `docs/V0.1_IMPLEMENTATION.md` 的剩余验收步骤，在获准安装 Edge 后补齐同一证据；Edge 通过后即可关闭 V0.1 浏览器矩阵。Native Host 的正式稳定目录、Developer ID 签名与公证属于后续 release spike，不是本任务关闭门槛；当前 `/tmp` 路径也不得当成发布方案。
