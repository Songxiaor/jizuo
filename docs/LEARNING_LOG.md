...
## 任务：管线可见 + 学习留痕 + 抓取队列 + 复制反馈（7/23 深夜—7/24 收官）

- 执行：Claude 主控。638 测试仅余 9 个已知基线失败；已部署日用 App（真实数据根）。
- **处理状态徽标**：列表行右下角三枚小图标（已转写/已总结/有脑图），列表 SQL 加三个 EXISTS 标志，行投影可选字段兼容旧序列化。
- **重复链接拦截**：手动添加已在库链接先弹提示（重复添加併入原条目），可「仍要重新抓取」；存储层 canonical 去重本就存在，补的是感知。
- **摘录+笔记（Migration011）**：`task_notes`/`task_excerpts` 新表；阅读区选中右键「添加到摘录」；笔记 800ms 防抖自动保存；批注卡位于标签编辑器上方；删任务级联清理。用户思考与机器产物分离，自动流程永不触碰。
- **微信抓取回归修复**：导航守卫把文章页站外 iframe（广告/腾讯视频）当主框架违规终止捕获；抽 `navigationDecision(url:isMainFrame:)` 纯函数——主框架离站=终止，子框架站外=静默拦截；7 条裁决测试钉死。
