...
| 博主分区 | 侧栏「今天」与「平台」之间可折叠博主区：全部博主、最多 5 位置顶、点博主筛选已存作品。用平台+作者 ID 关联，不用昵称合并；旧内容归属不明不强归类。加号粘贴抖音主页复用现有三列导入卡；其他平台不承诺主页抓取 |
...
2026-08-14 真机确认的实际主窗口在此结构上扩展：侧边栏含"全部/最近/待分类"与按平台的来源分类计数，以及"收藏"和"我的笔记"入口；详情页含脑图区块与"总结/翻译"操作；工具栏提供"添加链接"手动入口；导出菜单见 §5.3。随后在「今天」与「平台」之间增加可折叠博主分区：置顶博主筛选同一份已存列表，「全部博主」在中间列可搜索、置顶；主页抓取首版只复用抖音三列导入卡。
...
- 安装完整 App 后，设置的“连接与数据”分组出现“MCP 连接”。默认关闭；用户可分别允许“抓取与整理”“转写与总结”。
...
- 工具覆盖：连接状态、搜索和分段读取资料、提交内容链接、查询捕获结果、查询博主、发现抖音主页作品、继续/停止发现、保存明确选择的作品、下载视频、本机视频转写、处理状态、总结、标签、收藏和打开记录。
- 多个博主依次执行；当前平台主页能力仅抖音。发现结果不是已经入库；保存入队不是完成；必须查询已保存记录，再查询转写/生成终态。需要登录、验证、模型下载或数据发送授权时明确交回 App 处理。
