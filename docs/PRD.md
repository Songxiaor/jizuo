...
| 预置服务商 | 模型设置预置 12+ 家服务商及"自定义"选项，保持 Provider-neutral；Command Code 使用官方 Provider API，支持具备 API 权限的套餐额度与 Provider 按量付费，Go 套餐不支持。公开模型目录不代表密钥或套餐已验证，保存后由用户测试连接；Claude 自动走 Messages，其余模型走 Chat Completions，不作为在线语音转写服务 |
...
### 2026-09-08 全部来源平台统一卡片图库

来源平台（X、抖音、公众号、B站、GitHub、YouTube、Discourse、Reddit、Substack、小红书、其他）共用稳定左导航 + 右卡片网格；点卡片进入阅读，返回保留平台、滚动位置与卡片锚点。图库使用 `HistoryGallerySplitView`（HSplitView），与普通资料三栏入口统一固定导航宽为 `DesignTokens.Layout.sidebarIdeal`（约 220pt），避免平台与普通资料切换跳宽；不重构笔记编辑器。

再次点击同一来源平台保持筛选（幂等，仅在已处于同一平台图库导航态时）；从工作台/博主目录点同平台仍应回到图库。显式全部/清除筛选才退出。跨平台清除上个平台搜索词与阅读暂存；切换平台时清空旧列表避免标签与内容错位闪现。图库不自动勾选首条；进入阅读前暂存主动多选，返回时恢复（读前无选择则返回仍无选择），阅读目标与批量勾选分开。全部平台提供一致的轻量多选、取消选择、批量总结/翻译入口与右键菜单，复用既有忙碌、只读、费用确认与删除保护。

卡片共用 Shell：有封面显示封面，无封面显示文字预览；封面失败可解释且不等同无封面。缺作者不把平台名当作者。视频仅依据已存 `hasMedia` 标识；互动指标只展示已持久化真实值（含 0），缺失不塞「—」，公众号与非社交站点不硬套点赞条。列表按最近更新排列并在图库标明，缺发布时间显示「保存于」。卡片预览仅做展示层清洗，不改持久化原文或导出。

### 作品封面与展示排序（2026-09-08）

导入选择弹窗与博主作品页提供默认顺序、发布时间双向、点赞双向排序。仅重排已加载数据，博主页的本次批次与已有作品各组分别排序；不修改勾选、后台抓取队列或分页游标。未知值在两个方向都排最后，同值保留原顺序，真实 0 与缺失分开。只有月日的发现日期以本次浏览为基准取最近该日期用于排列，不将推算年份写入原文或展示为真实发布时间。

作品封面共用有界缩略图缓存和相同请求合并，默认最多 3 路加载、32 MiB / 64 项解码缓存；后台降采样，本地封面优先，远程读取继续使用现有安全通道。离屏取消不影响仍在使用同图的卡片；失败可重试，无封面的候选或批次卡提供文字预览。发现列表一轮合并后最多发布一次变更，后续出现的懒加载封面可补入，重复主页头信息不重复刷新。

X 同一作品的基础数据与全文补全读取允许结构化并发，但不并发操作共享 WebKit 或改写保存队列。全文与指标仍按原回退逻辑合并，不能为了速度将截断预览当完整原文保存；不承诺固定的真实网络提速比例。

### 主页抓取中栏的完成状态（2026-09-08）

中栏只展示尚未完成的任务标题、状态、取消或重试，不重复封面、发布时间及互动数据。全部成功后自动替换为「已保存 N 条」的简短提示，用户可关闭，关闭状态跨启动保留且不删除抓取记录或资料。失败、中断和取消项保留操作入口，不能被完成提示隐藏。完成不改变当前阅读选择；博主作品页继续保留统一卡片与原位完成行为。

### 本机素材库：本机来源与素材整理层（2026-09-23）

定位：汲作是内容创作系统的素材收件箱，创作系统通过 MCP 从这里取素材。本批新增三个本机来源和一层素材整理。

**本机来源**（侧栏「来源平台」出现「语音备忘录」「本地文件」「备忘录」三个分组，条目归「资料」，不归「我的笔记」）：

- 语音备忘录：「添加 → 同步语音备忘录」。需要用户在「系统设置 → 隐私与安全性 → 完全磁盘访问」里允许汲作；未授权时弹窗说明并给出打开系统设置的按钮。录音转成 M4A 复制进汲作媒体库，原录音不改动。列表时间是录制时间。按系统录音 ID 去重，只拉新录音；在汲作删掉的录音不会被下一次同步带回；只在 iCloud、没下载到本机的录音单独计数并说明。「最近删除」里的录音不导入。
- 本地文件：拖进主窗口，或「添加 → 导入本地文件…」（⌘⇧I）。文档（PDF、Word、RTF、TXT、Markdown）提取文字；图片保留原图并在正文顶部显示，附本机识别出的文字（识别不到文字也照样导入）；音频统一转 M4A、视频 mp4/mov 原样存入，可播放、可转写。按文件内容哈希去重；回收站里的文件再次拖入会被恢复。不支持的格式逐个说明原因，不拦截整批。
- 备忘录：「添加 → 同步备忘录」。通过 Apple 官方脚本接口只读，首次使用时系统会弹窗征求「自动化」授权。按原文件夹写入条目作者字段，列表可见、可搜索。有改动的备忘录再次同步会更新为新版本；内容未变的跳过；加密备忘录读不到正文，计数说明；「最近删除」文件夹不导入。正文保留标题、列表、粗体结构，内嵌图片不导入（显示为「[图片]」）。
- 导入一律不自动转写、不自动总结、不抢当前阅读焦点；是否花模型费用由用户在条目里决定。

**素材整理层**：

- 素材类型：预置「灵感、观点、案例、金句、数据、选题」六个类型，本质是普通标签，可在阅读页标签区一键切换，也可在列表右键「素材类型」中设置；侧栏标签筛选、搜索、MCP 均可用。
- 已使用：素材被创作系统用过后带「已使用」标签；侧栏新增「未使用」（资料中不带「已使用」的条目，含计数）。阅读页标签区和右键菜单都能手动标记或取消。
- MCP：`jizuo_search` 新增 `material_type`、`unused_only` 参数，结果带 `tags` 与 `used`；按 `material_type` 取素材时同时包含「我的笔记」（快速记录的灵感存在笔记里）；新增 `jizuo_mark_used(task_id, used_in)`，贴「已使用」并在条目笔记里追加「## 使用记录 / - 日期 已用于《…》」，同日同去处不重复；`jizuo_status` 返回可用素材类型。
- 快速记录：全局快捷键 ⌃⌥N（或菜单「文件 → 快速记录」）弹出小窗，写一句、选素材类型（默认灵感），⌘↩ 保存到「我的笔记」并贴类型标签，不切走当前 App 的工作。快捷键被其它 App 占用时只能用菜单。

**明确不做**：微信聊天记录/收藏（无官方读取途径）；备忘录内嵌图片与附件；自动定时同步。
