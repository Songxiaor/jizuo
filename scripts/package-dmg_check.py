#!/usr/bin/env python3
"""Focused regression check for architecture-specific DMG app thinning.

Exercises only the file-layout boundary: the main executable is thinned, while
NativeHost remains the verified universal package created earlier in the build.
No Swift build, signing, or DMG creation is invoked.
"""
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="package-dmg-check.") as temporary:
        work = Path(temporary)
        ...
        source_host = source_host_root / "LinkDigestNativeHost"
        ...
        source_host.write_bytes(b"universal host binary")
        ...
        def fake_run(*command: str, cwd: Path | None = None) -> None:
            del cwd
            check(command[:2] == ("/usr/bin/lipo", str(source_executable)) or command[:2] == ("/usr/bin/lipo", str(destination_executable)), "only the main app executable may be passed to lipo")
        ...
        def fake_verify(package: Path, root: Path) -> object:
            ...
            check(package.joinpath("LinkDigestNativeHost").read_bytes() == original_host_bytes, "NativeHost executable must remain universal and byte-identical")
        ...
        check(destination_host_root.joinpath("LinkDigestNativeHost").read_bytes() == original_host_bytes, "NativeHost must not be lipo-thinned or replaced")
        check([kind for kind, _ in events] == ["lipo", "archs", "verify"], "NativeHost verification must run after main app thinning")
